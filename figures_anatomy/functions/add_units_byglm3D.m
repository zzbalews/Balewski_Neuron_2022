function [AP_slices,track_coords_disc,track_coords_size] = add_units_byglm3D(...
    ses_info,dir_hdd,dir_info,show_region,glmdata)
...
for S = 1:length(subject_names)
    ...
    flat_disc = nan(size(MLAPDV));
    for i = 1:3
        low = floor(min(MLAPDV(:,i)));
        high = ceil(max(MLAPDV(:,i)));
        
        if S==2
            low = low-0.5;
        end
        
        flat_disc(:,i) = make_disc(MLAPDV(:,i), [low:high]);
    end
    ...
    for j = 1:3
        ...
        scatter3(coords_disc(:,1),coords_disc(:,2),coords_disc(:,3),...
