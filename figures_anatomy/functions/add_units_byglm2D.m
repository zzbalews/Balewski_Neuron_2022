function add_units_byglm(...
    ses_info,dir_hdd,dir_info,show_region,glmdata,collapse, subject)

% subject_names = unique(ses_info{:,2});
subject_names = {subject};

ses_info = ses_info(strcmp(ses_info.subject, subject),:);
...
for S = 1:length(subject_names)
    ...
    % ignore 'collapse' dim
    if strcmp(collapse, 'ML')
        dims_idx = [2,3];
        dims_names = {'AP','DV'};
        dims_lim = [21 32; 18 29];
    elseif strcmp(collapse, 'AP')
        dims_idx = [1,3];
        dims_names = {'ML','DV'};
        dims_lim = [1.5 9; 18 29];
    elseif strcmp(collapse,'DV')
        dims_idx = [1,2];
        dims_names = {'ML','AP'};
        dims_lim = [1.5 9; 21 32];
...
%         end
        ...
        flat_disc(:,i) = make_disc(flat_raw(:,i), [low:0.5:high]);
    end
...
%         clrs = (1-coords_sig(:,j)) + coords_sig(:,j)*clrs_max(j,:);
        ...
        cmap_idx = round(rescale(coords_sig(:,j), 1, 256));
        
        scatter(coords_disc(:,1),coords_disc(:,2),...
            coords_size,...
            cmap(cmap_idx,:), 'filled',...
            'MarkerFaceAlpha',0.8,'Marker',marker_shape{S},...
            'MarkerEdgeAlpha',0);
...
%         text(dims_lim(1,1)+1,dims_lim(2,2)-(S/2),[subject(1),': model p=',num2str(round(model_p(j),3))])
        ...
        ylabel(dims_names{2})
%         
%         ax_AP = find(strcmp(dims_names,'AP');
%         ax_DV = find(strcmp(dims_names,'DV');
%         ax_ML = find(strcmp(dims_names,'ML');
