function show_responsetime_resid_vsvalue(bhvdata,variable,which_value)
...
for s = 1:length(subject_names)
    ...
    for i = 1:nmids
        ...
        track_y_resid_aftervalmax(i,:) = [...
            nanmean(y_resid_aftervalmax(idx)),...
            nanstd(y_resid_aftervalmax(idx))./sqrt(N)];
        ...
        track_y_resid_aftervaldiff(i,:) = [...
            nanmean(y_resid_aftervaldiff(idx)),...
            nanstd(y_resid_aftervaldiff(idx))./sqrt(N)];
    end
    ...
    a=shadedErrorBar(valdiff_midpoints,track_y_resid_aftervalmax(:,1),...
        ...
        'lineprops',{'Color',[0 .4 .8],'LineWidth',2,'LineStyle',style});
    xlabel(['\Delta',which_value])
    ylabel({['residual ',variable,' (ms)'],['(after max',which_value,')']})
    title(subject)
    ...
    ylabel({['residual ',variable,' (ms)'],['(after \Delta',which_value,')']})
    title(subject)
    ...
    Y = [50*floor(min(track_y(:,1))/50),50*ceil(max(track_y(:,2))/50)];
    for i = [0,2]
        subplot(2,2,i+s)
        ylim(Y);
    end
