...
w_names(end+1:end+2) = {'softmax w2*deltaval','softmax side bias'};

loglikefunc = @(w) ...
    -sum(log(softmax_discount(functype,{TR.amnt,TR.prob},w) .^ TR.N(:,1))) + ...
    -sum(log((1-softmax_discount(functype,{TR.amnt,TR.prob},w)) .^ TR.N(:,2)));
...
goleft_fit = softmax_discount(functype,{V_delta},w_fit);
