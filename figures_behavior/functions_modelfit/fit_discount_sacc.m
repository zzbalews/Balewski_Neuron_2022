function [w_names,w_fit,BIC,R2] = ...
    fit_discount_sacc(functype,lever,jpg,amnt,prob,firstsacc)

%% reduce to unique trial rows
% by amnt/prob offer pair + first saccade location
TR = struct();
 
temp = unique([jpg,firstsacc],'rows');
TR.jpg = temp(:,1:2);
TR.firstsacc = temp(:,3);
...
w_names(end+1:end+3) = {'softmax w2*deltaval','softmax w3*firstsacc','softmax side bias'};

loglikefunc = @(w) ...
    -sum(log(softmax_discount_sacc(functype,{TR.amnt,TR.prob},TR.firstsacc,w) .^ TR.N(:,1))) + ...
    -sum(log((1-softmax_discount_sacc(functype,{TR.amnt,TR.prob},TR.firstsacc,w)) .^ TR.N(:,2)));
...
goleft_fit = softmax_discount_sacc(functype,{V_delta},firstsacc,w_fit);
